...
## Order of work
Built (25), all five series: colour spaces, why HSL lies, communication and type, the typography genome, keypress to pixel, the gamuts, what a CSS property costs, the event loop (for interfaces), Big O (in the keystroke), every named CSS colour, how a URL becomes a page, easing versus springs, why z-index 9999 does nothing, contrast two ways (WCAG 2 vs APCA), specificity three columns, seven and the planetary hours (MA), thirty years of CSS layout, why 16px is not 16px, the four lengths of a string (Unicode), floating point at the pixel, what a byte of JavaScript costs, your commit is not gone (git), getting the menu out of the card (anchor positioning), the anatomy of a letter, why they are still seeing the old version (caching). Series: Colour and perception 6, Typography 4, The browser and the machine 13, Motion and interaction 1, Number and symbol 1. Every chart has 2 to 5 follow-up facts in facts.ts for the X lane. Deployed 2026-09-22.
