...
## Order of work
Built (21): colour spaces, why HSL lies, communication and type, the typography genome, keypress to pixel, the gamuts, what a CSS property costs, the event loop (for interfaces), Big O (in the keystroke), every named CSS colour, how a URL becomes a page, easing versus springs, why z-index 9999 does nothing, contrast two ways (WCAG 2 vs APCA), specificity three columns, seven and the planetary hours (MA), thirty years of CSS layout, why 16px is not 16px (measured x-heights), the four lengths of a string (Unicode), floating point at the pixel, what a byte of JavaScript costs. Next: git as a graph → anchor positioning and the top layer → the anatomy of a letter (opentype.js outlines) → caching and the four headers → then alternate one from each series.
